...
const defaultFacilities = [
  "Điều hoà", "Bếp", "Khăn", "Bồn tắm", "TV", 
  "WiFi", "Quầy bar", "Ban công"
];
...
export default function Room({ rooms, rates, filter }: Props) {
  ...
  const columns: Column<Room>[] = [
    { header: "Số phòng", accessor: (room: Room) => disPlayRoomNumber(room.room_number, room.floor) },
    { 
      header: "Loại giường", 
      accessor: (room: Room) => {
    ...
    },
    {
      ...
      accessor: (room: Room) => {
        const statusMap: Record<string, { text: string; color: string }> = {
          ...
          blocked: { text: "Không khả dụng", color: "bg-gray-100 text-gray-800" }
        };
  ...
  const editRoom = (room: Room) => {
    ...
    if (room.facilities) {
      setSelectedFacilities(room.facilities.split(", ").filter(f => f.trim()));
    } else {
      setSelectedFacilities([]);
    }
  ...
  const getFilterCounts = () => {
    ...
    const available = rooms.data.filter(room => room.status === 'available').length;
    const booked = rooms.data.filter(room => ['booked', 'reserved'].includes(room.status)).length;
        ...
        <Modal show={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <form onSubmit={submit} className="p-6">
            ...
            <div className="mt-4 flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1">
                <InputLabel htmlFor="room_number" value="Số phòng" />
                <TextInput
                  id="room_number"
                  type="number"
